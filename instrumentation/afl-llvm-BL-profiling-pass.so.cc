...
#include "PathNumbering.h"
  ...
  /* this is requried for PREV_LOC_T, but I am using 64 bits ints so I need to
   * redefine it */
...
class PathProfiler : public ModulePass {
 ...
 private:
  ...
  uint32_t ctx_k = 0;
  const char *ctx_str = NULL, *caller_str = NULL;
  ...
  /* K-ctx vector size (must be a power of two) */
  ...
  unsigned int insts = 0; /* Keep track of the emplaced instrumentations */
...
// #endif
  ...
  void edgeInstrument(Function &F, Module &M);
  ...
  ConstantInt *toConstInt(long val, int bitsize);
...
static cl::opt<bool> DotPathDag(
    "path-profile-pathdag", cl::Hidden,
    cl::desc("Output the path profiling DAG for each function."),
    cl::init(false));

static cl::opt<uint64_t> HashThreshold(
    "path-profile-hash-threshold", cl::Hidden,
    cl::desc("Set the hash table threshold."), cl::init(0xFFFFFFFFFFFFFFFF));
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "PathProfiler", "v0.1",
          ...
          [](PassBuilder &PB) {
            ...
            PB.registerOptimizerLastEPCallback(
                [](ModulePassManager &MPM, OptimizationLevel OL) {
                  if (getenv("PATH_DEBUG"))
                    oprint("Opt: -O" << OL.getSpeedupLevel());
...
BLInstrumentationNode::BLInstrumentationNode(BasicBlock *BB)
    : BallLarusNode(BB),
      _startingPathNumber(nullptr),
      _endingPathNumber(nullptr),
      _pathPHI(nullptr) {
}
...
Constant *BLInstrumentationDag::getCounterArray() {
  return _counterArray;
}

void BLInstrumentationDag::setCounterArray(Constant *c) {
  _counterArray = c;
}
...
void BLInstrumentationDag::unlinkEdge(BallLarusEdge *edge) {
  if (edge == getExitRootEdge())
    oprint(" Removing exit->root edge\n");
...
void PathProfiler::preparePHI(BLInstrumentationNode *node) {
  ...
  PHINode *phi = PHINode::Create(Int64Ty, std::distance(PB, PE), "pathNumber",
                                 &*insertPoint);
  node->setPathPHI(phi);
...
void PathProfiler::pushValueIntoNode(BLInstrumentationNode *source,
  ...
  if (target->getNumberPredEdges() <= 1) {
    ...
    // oprint("  Passing path number"
    //                   << (source->getEndingPathNumber() ? "" : " (null)")
    //                   << " value through.\n");
  } else {
    if (target->getPathPHI() == nullptr) {
      // oprint("  Initializing PHI node for block '"
      //                   << target->getName() << "'\n");
      preparePHI(target);
    }
    pushValueIntoPHI(target, source);
    // oprint("  Passing number value into PHI for block '"
    //                   << target->getName() << "'\n");
  }
}
...
// The Value* in node, oldVal,  is updated with a Value* corresponding to
// oldVal + addition. {Commented because it's not used}
/*void PathProfiler::insertNumberIncrement(BLInstrumentationNode *node,
  ...
  // oprint("  Creating addition instruction.\n");
  Value *newpn =
...
void createPrintfCall(IRBuilder<> &builder, Value *reg, Function &F,
                      std::string prep = "") {
  ...
  std::vector<Type *> printfArgsTypes(
      {Type::getInt8PtrTy(context), IntegerType::getInt64Ty(F.getContext())});
  FunctionType *printfType =
      FunctionType::get(int64Type, printfArgsTypes, true);
  auto printfFunc = module->getOrInsertFunction("printf", printfType);

  // The format string for the printf function, declared as a global literal
  // Value *str = builder.CreateGlobalStringPtr("[PATH-DEBUG] Func: "+
  // F.getName().str() + ")\t" + prep + "\t%ld\n", "str");
  Value *str = builder.CreateGlobalStringPtr(
      "[PDB] Func:" + F.getName().str() + "\t" + prep + "%ld\n", "str");

  std::vector<Value *> argsV({str, reg});
  CallInst *printCall = builder.CreateCall(printfFunc, argsV, "calltmp");
  printCall->setMetadata(module->getMDKindID("nosanitize"),
...
void PathProfiler::insertCounterIncrement(
    Value *incValue,  // this should be the path number - It's a ConstantInt,
                      // originally used as index in the array of counters
    Instruction *insertPoint, BLInstrumentationDag *dag, bool increment) {
  // oprint("insertCounterIncrement");
  //  XXX hack to handle calls to exit, abort, etc. 
  Instruction *origPnt = insertPoint;

  if (isa<UnreachableInst>(insertPoint)) {
    if (getenv("PATH_DEBUG")) {
      oprint("IsUnr - gettingPrevNonDbg");
      oprint(*insertPoint);
      oprint(*(insertPoint)->getParent());
    }
    // oprint(*(insertPoint)->getParent()->getParent());
    insertPoint = insertPoint->getPrevNonDebugInstruction(
        /*SkipPseudoOp=*/true);

    if (insertPoint == nullptr) {
      if (getenv("PATH_DEBUG"))
        oprint("PrevNonDbg is null - resetting to origPnt");
      insertPoint = origPnt;
    }
  }
  ...
  Function *func = insertPoint->getFunction();

  /* Retrieve the current function's address */
  Value *funAddr =
      ConstantExpr::getBitCast(func, Type::getInt8PtrTy(func->getContext()));
  IRB.CreateStore(funAddr, funcAddr)
      ->setMetadata(thisModule->getMDKindID("nosanitize"),
                    MDNode::get(*Context, None));

  LoadInst *load = IRB.CreateLoad(
#if LLVM_VERSION_MAJOR >= 14
      Type::getInt8PtrTy(func->getContext()),
#endif
      funcAddr, "funaddr");
  load->setMetadata(thisModule->getMDKindID("nosanitize"),
                    MDNode::get(*Context, None));

  Value *ptrToInt = IRB.CreatePtrToInt(
      load, IRB.getInt64Ty());  // Convert the function's address to an int

  /* New version - To test later */
  // Value* funAddr = ConstantExpr::getBitCast(func,
  // Type::getInt8PtrTy(func->getContext())); Value* ptrToInt =
  // IRB.CreatePtrToInt(funAddr, IRB.getInt64Ty());
...
#endif
      AFLMapPtr);
  MapPtr->setMetadata(thisModule->getMDKindID("nosanitize"),
                      MDNode::get(*Context, None));

  /* Our hash function: (PathID(incValue) ^ FunctionID(function address)) %
   * MAP_SIZE */
  ConstantInt *mapSize64 = ConstantInt::get(IRB.getInt64Ty(), map_size);
  Value *Xor = IRB.CreateXor(ptrToInt, incValue);
  Value *Mod = IRB.CreateSRem(
      Xor,
      mapSize64);  // We do this to ensure the Xor stays within the map size
  ...
  if (getenv("PATH_DEBUG_PRINTS")) {
    // createPrintfCall(IRB, Xor, *func, "FuncID^PathId ");
    // createPrintfCall(IRB, Mod, *func, "INDEX [FuncID^PathId mod MapSize] ");
    // createPrintfCall(IRB, ptrToInt, *func, "FuncID ");
    if (increment)
      createPrintfCall(IRB, incValue, *func, "Path#:");
    // createPrintfCall(IRB, mapSize64, *func, "MapSize: ");
  }

  Value *Incr;
  if (!getenv("PATH_NO_HITCOUNTS")) {
    LoadInst *Counter = IRB.CreateLoad(
#if LLVM_VERSION_MAJOR >= 14
        IRB.getInt8Ty(),
#endif
        MapPtrIdx);
    Counter->setMetadata(thisModule->getMDKindID("nosanitize"),
                         MDNode::get(*Context, None));

    // This is the way CLASSIC mode increments the counter
    Incr = IRB.CreateAdd(Counter, increment ? One : mOne);
  }
  // Value *inc = increment ? One : mOne;
  // CallInst *Incr = IRB.CreateCall(UAddSatIntrinsic, {Counter, inc}, "newPc");
...
void PathProfiler::insertInstrumentationStartingAt(BLInstrumentationEdge *edge,
  ...
  if (sourceNode->getBlock() && sourceNode->getNumberSuccEdges() <= 1) {
    // oprint("  Potential instructions to be placed in: "
  ...
  // Somehow, splitting must have failed.
    ...
    if (getenv("PATH_DEBUG")) {
      ...
      oprint("  Couldn't split edge " << (*edge) << ".\n");
    }
    return;
  }
  ...
  if (edge->getType() == BallLarusEdge::BACKEDGE ||
    ...
    assert(top->isInitialization() &&
           " Top phony edge did not"
  ...
  for (BLEdgeIterator next = targetNode->succBegin(),
    ...
    if (!((BLInstrumentationEdge *)(*next))->hasInstrumentation()) {
      ...
      // oprint("  Edge " << *(BLInstrumentationEdge *)(*next)
...
void PathProfiler::insertInstrumentation(BLInstrumentationDag &dag, Module &M) {
  ...
  for (BLEdgeIterator edge = callEdges.begin(), end = callEdges.end();
    ...
    // oprint("   Path number initialized: "
    ...
    if (node->getStartingPathNumber()) {
      ...
      if (inc)
        newpn = BinaryOperator::Create(
            ...
            createIncrementConstant(inc, 64), "pathNumber", &*insertPoint);
      else
...
// Return true if `F` has an available_externally linkage (i.e. equivalent to a
// declaration)
...
bool PathProfiler::runOnFunction(Function &F, Module &M) {
  if (getenv("PATH_DEBUG"))
    oprint("runOnFunction: " << F.getName());
  // Build DAG from CFG
  ...
  /*if (dag.getNumberOfPaths() > HashThreshold) {
    fprintf(stderr, "Too many paths in %s to track in an array. Skipping",
          F.getName().str().c_str());
    return false;
  }*/
  ...
  auto *FunctionEntry = ConstantStruct::get(
      FTEntryTy, createIncrementConstant(dag.getNumberOfPaths(), 64),
  ...
  auto *Counter = ConstantExpr::getInBoundsGetElementPtr(
      FTEntryTy, FTEntryGV,
      ArrayRef<Constant *>(
          {Constant::getNullValue(Int64Ty), ConstantInt::get(Int64Ty, 1)}));
  ...
  */
  ...
  if (getenv("PATH_DEBUG") && !getenv("AFL_LLVM_CMPLOG")) {
    // oprint("[PATH-DEBUG] Function: " << F.getName() << " - Paths: " <<
    // dag.getNumberOfPaths()); errs().flush();
    std::string demangled = demangle(F.getName().str().c_str());
    std::ofstream myfile;
    myfile.open("/tmp/static_paths.txt", std::ios_base::app);
    if (myfile.is_open()) {
      // std::string fname = F.getName().str();
      std::string demangled = demangle(F.getName().str().c_str());
      // std::string mystr = "" + demangled + "\t" +
      // std::to_string(dag.getNumberOfPaths());
      std::string mystr = "" + F.getName().str() + "\t" +
                          std::to_string(dag.getNumberOfPaths());
      oprint("Trying to write: " << mystr);
      myfile << mystr << std::endl;
      myfile.close();
      mystr = "";
    } else
      oprint("ERROR: Could not open file!");

    oprint("[PDS] " << demangled << "\t"
                    << std::to_string(dag.getNumberOfPaths()));
    // oprint("[PDS] " << F.getName() << "\t" <<
    // std::to_string(dag.getNumberOfPaths()));
...
ConstantInt *PathProfiler::toConstInt(long val, int bitsize) {
  return ConstantInt::get(IntegerType::get(*this->Context, bitsize), val);
}

void PathProfiler::edgeInstrument(Function &F, Module &M) {
  if (getenv("PATH_DEBUG"))
    oprint("Instrumenting " << F.getName().str()
                            << " with the edge coverage instrumentation");
  ...
  for (auto &BB : F) {
    ...
    if (instrument_ctx && &BB == &F.getEntryBlock()) {
      ...
      if (ctx_k) {
        ...
        PrevCaller->setMetadata(M.getMDKindID("nosanitize"),
                                MDNode::get(*Context, None));
        PrevCtx =
            IRB.CreateZExt(IRB.CreateXorReduce(PrevCaller), IRB.getInt64Ty());
        // PrevCtx = IRB.CreateZExt(IRB.CreateXorReduce(PrevCaller),
        // IRB.getInt32Ty());
      ...
      if (has_calls) {
        ...
        Value *NewCtx = ConstantInt::get(Int64Ty, AFL_R(map_size));
#ifdef AFL_HAVE_VECTOR_INTRINSICS
        if (ctx_k) {
          Value *ShuffledPrevCaller = IRB.CreateShuffleVector(
              PrevCaller, UndefValue::get(PrevCallerTy), PrevCallerShuffleMask);
          Value *UpdatedPrevCaller =
              IRB.CreateInsertElement(ShuffledPrevCaller, NewCtx, (uint64_t)0);
...
#endif
      CurLoc = ConstantInt::get(Int64Ty, cur_loc);
    ...
    // Value *PrevLocTrans = PrevLoc;
    // PrevLocTrans = IRB.CreateZExt(PrevLocTrans, IRB.getInt32Ty());
    // PrevLocTrans = IRB.CreateZExt(PrevLocTrans, IRB.getInt64Ty());
    ...
    if (ngram_size)
      ...
      // IRB.getInt32Ty());
      PrevLocTrans =
          IRB.CreateZExt(IRB.CreateXorReduce(PrevLoc), IRB.getInt64Ty());
    else
#endif
    ...
    if (instrument_ctx)
      PrevLocTrans =
          IRB.CreateZExt(IRB.CreateXor(PrevLocTrans, PrevCtx), Int64Ty);
    // PrevLocTrans = IRB.CreateZExt(IRB.CreateXor(PrevLocTrans, PrevCtx),
    // Int32Ty);
    else
      PrevLocTrans = IRB.CreateZExt(PrevLocTrans, IRB.getInt64Ty());
    // PrevLocTrans = IRB.CreateZExt(PrevLocTrans, IRB.getInt32Ty());
    ...
    if (ngram_size)
      MapPtrIdx = IRB.CreateGEP(
          Int8Ty, MapPtr,
          IRB.CreateZExt(
              IRB.CreateXor(PrevLocTrans, IRB.CreateZExt(CurLoc, Int64Ty)),
              Int64Ty));
    // IRB.CreateZExt(IRB.CreateXor(PrevLocTrans, IRB.CreateZExt(CurLoc,
    // Int32Ty)), Int32Ty));
    else
#endif
      MapPtrIdx = IRB.CreateGEP(
    ...
    IRB.CreateStore(Incr, MapPtrIdx)
        ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(*Context, None));
...
#if LLVM_VERSION_MAJOR >= 11 /* use new pass manager */
PreservedAnalyses PathProfiler::run(Module &M, ModuleAnalysisManager &MAM) {
#else
bool PathProfiler::runOnModule(Module &M) {
  ...
  if ((isatty(2) && !getenv("AFL_QUIET")) || getenv("AFL_DEBUG") != NULL) {
    SAYF(cCYA
         "afl-llvm-BL-profiling-pass" VERSION cRST
         " by <giacomo.priamo@uniroma1.it> and <adrian.herrera@anu.edu.au>\n");
  ...
  // FTEntryTy = StructType::create(*Context, {Int64Ty, VoidPtrTy}, "FTEntry");

  One = ConstantInt::get(Int8Ty, 1);
  mOne = ConstantInt::get(Int8Ty, -1);
  ...
  M.getOrInsertGlobal(
      "_func_addr",
      PointerType::get(IntegerType::getInt8Ty(M.getContext()), 0));
  funcAddr = M.getNamedGlobal("_func_addr");
  funcAddr->setLinkage(llvm::GlobalValue::CommonLinkage);
  // funcAddr->setLinkage(llvm::GlobalValue::LinkOnceAnyLinkage); 
  funcAddr->setInitializer(Constant::getNullValue(
      PointerType::get(IntegerType::getInt8Ty(M.getContext()), 0)));
  ...
  //                           PointerType::getUnqual(FTEntryTy), Int64Ty)
  //                     : nullptr;

  /*
  //Apparently this crashes on LLVM 12 (prolly due to aliased lib functions)
  std::set<StringRef> builtins;
  //Gather all built-in functions (to avoid instrumenting them later)
  if (!getenv("AFL_DONT_OPTIMIZE")) {



    oprint("Collecting built-in functions...");
    const TargetLibraryInfo *TLI;
    if (NULL == TLI || nullptr == TLI )
      oprint("**TLINULL");
    LibFunc inbuilt_func;
      for (Function &F : M) {
        oprint("**fn: " << F.getName());
        if (TLI->getLibFunc(F.getName(), inbuilt_func)) {
          oprint("** here");
          builtins.insert(F.getFunction().getName());
        }
      }
    oprint("Collected " << std::to_string(builtins.size()) << " built-in
  functions.");
  }
  */
  ...
  /* Also use classic mode instrumentation alongside the pathprofiling one */

  unsigned functionNumber = 0;

  bool haveHarness = false;

  /* Check whether any functions should be instrumented with edge coverage
   * instrumentation */
  std::set<std::string> edgeFuncsNames;
  char *edgeFuncs = getenv("PATH_EDGE_INSTRUMENT");
  if (edgeFuncs) {
    if (getenv("PATH_DEBUG"))
      oprint("Enabling edge coverage instrumentation");

#ifdef AFL_HAVE_VECTOR_INTRINSICS
    IntLocTy = IntegerType::getIntNTy(*Context, sizeof(PREV_LOC_T) * CHAR_BIT);
#endif
    ...
    if (!ngram_size_str)
      ngram_size_str = getenv("AFL_NGRAM_SIZE");
    char *ctx_k_str = getenv("AFL_LLVM_CTX_K");
    if (!ctx_k_str)
      ctx_k_str = getenv("AFL_CTX_K");
    ctx_str = getenv("AFL_LLVM_CTX");
    caller_str = getenv("AFL_LLVM_CALLER");

    instrument_ctx = ctx_str || caller_str;

#ifdef AFL_HAVE_VECTOR_INTRINSICS
    if (getenv("PATH_DEBUG"))
      oprint("Using vector intrinsics");
    ...
    if (ctx_k_str)
      if (sscanf(ctx_k_str, "%u", &ctx_k) != 1 || ctx_k < 1 ||
          ctx_k > CTX_MAX_K)
    ...
    if (ctx_str || caller_str)
      AFLContext = new GlobalVariable(
          M, Int64Ty, false, GlobalValue::ExternalLinkage, 0, "__afl_prev_ctx",
...
#endif
      AFLPrevCaller =
          new GlobalVariable(M, Int64Ty, false, GlobalValue::ExternalLinkage, 0,
                             "__afl_prev_caller", 0,
                             GlobalVariable::GeneralDynamicTLSModel, 0, false);

      /* AFLPrevCaller = new GlobalVariable(
          M, Int32Ty, false, GlobalValue::ExternalLinkage, 0,
         "__afl_prev_caller", 0, GlobalVariable::GeneralDynamicTLSModel, 0,
         false);
      */
    ...
    SmallVector<Constant *, 32> PrevLocShuffle = {UndefValue::get(Int64Ty)};

    for (unsigned I = 0; I < PrevLocSize - 1; ++I)
      PrevLocShuffle.push_back(ConstantInt::get(Int64Ty, I));
    // PrevLocShuffle.push_back(ConstantInt::get(Int32Ty, I));

    for (int I = PrevLocSize; I < PrevLocVecSize; ++I)
      PrevLocShuffle.push_back(ConstantInt::get(Int64Ty, PrevLocSize));
    // PrevLocShuffle.push_back(ConstantInt::get(Int32Ty, PrevLocSize));

    PrevLocShuffleMask = ConstantVector::get(PrevLocShuffle);

    SmallVector<Constant *, 32> PrevCallerShuffle = {UndefValue::get(Int64Ty)};
    // SmallVector<Constant *, 32> PrevCallerShuffle =
    // {UndefValue::get(Int32Ty)};

    if (ctx_k) {
      for (unsigned I = 0; I < PrevCallerSize - 1; ++I)
        PrevCallerShuffle.push_back(ConstantInt::get(Int64Ty, I));
      // PrevCallerShuffle.push_back(ConstantInt::get(Int32Ty, I));

      for (int I = PrevCallerSize; I < PrevCallerVecSize; ++I)
        PrevCallerShuffle.push_back(ConstantInt::get(Int64Ty, PrevCallerSize));
    ...
    size_t Pos = 0;
    std::string edgeFuncsString = std::string(edgeFuncs);
    std::string Token;
    std::string Delimiter(":");

    while ((Pos = edgeFuncsString.find(Delimiter)) != std::string::npos) {
      Token = edgeFuncsString.substr(0, Pos);
      edgeFuncsNames.insert(Token);
      edgeFuncsString.erase(0, Pos + Delimiter.length());

      if (getenv("PATH_DEBUG"))
        oprint("Adding " << Token << " to the edge instrument list");
    }

    if (!edgeFuncsString.empty()) {
      edgeFuncsNames.insert(edgeFuncsString);

      if (getenv("PATH_DEBUG"))
        oprint("Adding " << edgeFuncsString << " to the edge instrument list");
    }
  }

  for (Function &F : M) {
    if (F.isDeclaration() || F.isIntrinsic() || isAvailableExternally(F))
      continue;

    llvm::StringRef FNameR = F.getName();
    std::string FName = FNameR.str();

    if (FName.find(".module_ctor") != std::string::npos)
      continue;  // Should not instrument sanitizer init functions. (Taken from
                 // PCGuard)
    if (FNameR.startswith("__sanitizer_"))
      continue;  // Don't instrument __sanitizer_* callbacks. (Taken from
                 // PCGuard)
    if (F.getLinkage() == GlobalValue::AvailableExternallyLinkage)
      continue;  // Don't instrument MSVC CRT configuration helpers. They may
                 // run before normal init (Taken from PCGuard)
    if (FName == "__local_stdio_printf_options" ||
        FName == "__local_stdio_scanf_options")
      continue;  // (Taken from PCGUard)

    if (FNameR.startswith("LLVM")) {
      if (FName == "LLVMFuzzerTestOneInput")
        haveHarness = true;
      continue;
    }  // Skip LLVM functions (including the harness)

    if (F.getName() == "main" &&
        haveHarness)  // Skip the main added by the driver
      continue;
    ...
    if (!getenv("AFL_DONT_OPTIMIZE")) {
      /*
      //Apparently this crashes (prolly due to aliased lib functions) on LLVM 12
      if (builtins.count(F.getFunction().getName()) > 0) {
        oprint("Skipping built-in function: " << F.getFunction().getName());
        continue;
      }*/

      // TODO: find a more elegant solution (or switch to LLVM > 12 to make the
      // other solution work)
      std::string demangled = demangle(F.getName().str().c_str());
      // oprint("Demangled: " << demangled);
      if (demangled.find("std::") != std::string::npos ||
          demangled.find("__gnu") != std::string::npos ||
          demangled.find("__clang") != std::string::npos) {
        // oprint("Skipping std func: " << demangled);
        continue;
      }
    }
    ...
    if (getenv("PATH_EDGE_INSTRUMENT") &&
        edgeFuncsNames.find(F.getName().str()) != edgeFuncsNames.end()) {
      if (getenv("PATH_DEBUG"))
        oprint("Instrumenting " << F.getName().str() << " for edge coverage");
  ...
  if (getenv("PATH_DUMP_NUMBERS")) {
    oprint("\t[FN] Number of functions: " << functionNumber << "\n");
  }
  ...
  if (!be_quiet) {
    if (!insts)
      ...
      OKF("Instrumented %d locations (%s mode, ratio %u%%).", insts, modeline,
          inst_ratio);
    }
  }

  /* [MOD] This is useless -- removed */
...
static RegisterStandardPasses RegisterPathProfiler(
    PassManagerBuilder::EP_OptimizerLast, registerPathProfiler);
static RegisterStandardPasses registerPathProfiler0(
    PassManagerBuilder::EP_EnabledOnOptLevel0, registerPathProfiler);
#endif