...
static void edit_params(u32 argc, char **argv, char **envp) {
  ...
  OKF("USING NEW PM");
        ...
        cc_params[cc_par_cnt++] = "-fexperimental-new-pass-manager";
  #endif
        // [PATH_PROFILING] Choose bw Classic, Path and Selective Path
        WARNF("Choosing between CLASSIC, Selective PATH and PATH");
        if (getenv("AFL_PATH_PROFILING")) {
          OKF("PATH");
          // if (getenv("PATH_NOINLINE"))
          //   cc_params[cc_par_cnt++] = alloc_printf("-fpass-plugin=%s/RemoveInline.so", obj_path);
          cc_params[cc_par_cnt++] = alloc_printf("-fpass-plugin=%s/afl-llvm-BL-profiling-pass.so", obj_path);
        } else {
          OKF("CLASSIC");
...
#else
        OKF("USING OLD PM");
        // [PATH_PROFILING] Choose bw Classic and Path
        cc_params[cc_par_cnt++] = "-Xclang";
        cc_params[cc_par_cnt++] = "-load";
        cc_params[cc_par_cnt++] = "-Xclang";
        WARNF("Choosing between CLASSIC, Selective PATH and PATH");
        if (getenv("AFL_PATH_PROFILING")) {
          OKF("PATH");
          cc_params[cc_par_cnt++] = alloc_printf("%s/afl-llvm-BL-profiling-pass.so", obj_path);
        } else {
          OKF("CLASSIC");
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (strncmp(argv[i], "--afl", 5) == 0) {
      ...
      if (strncasecmp(ptr, "LTO", 3) == 0) {
        ...
        compiler_mode = LLVM;
        instrument_mode = INSTRUMENT_LLVMNATIVE;
        WARNF("Using native instrumentation");
