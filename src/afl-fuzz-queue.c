...
void cull_queue(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items; i++) {
    ...
    // [PATH_PROFILING] Clear the seeds.favored file
    if (getenv("PATH_DUMP_FAVORED")) {
      //printf("Unsetting favorite on %s\n", afl->queue_buf[i]->fname);
      fd = open("./seeds.favored", O_WRONLY | O_CREAT | O_TRUNC, DEFAULT_PERMISSION);
      if (unlikely(fd < 0)) { PFATAL("Unable to open './seeds.favored'"); }
      close(fd);
      //fflush(stdout);
    }
  }
  ...
  for (i = 0; i < afl->fsrv.map_size; ++i) {
    ...
    if (afl->top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      if (!afl->top_rated[i]->favored) {
        ...
        // [PATH_PROFILING] Dump the favored seeds (used by the queue culling strategy)
        if (getenv("PATH_DUMP_FAVORED")) {
          //printf("Set %s as favored!\n", afl->top_rated[i]->fname);
          fd = open("./seeds.favored", O_WRONLY | O_CREAT | O_APPEND, DEFAULT_PERMISSION);
          if (unlikely(fd < 0)) { PFATAL("Unable to open './seeds.favored'"); }
          u8 *str = alloc_printf("%s\n", afl->top_rated[i]->fname);
          ck_write(fd, str, strlen(afl->top_rated[i]->fname)+1, "seeds.favored");
          close(fd);
          //fflush(stdout);
        }
